...
def preprocess(dataset, time_past=0.05, time_future=0.25, time_interval=0.005, samples_past=100, samples_future=500):
    ...
    if dataset in preprocessor_neutouch:
        signals, labels, trials = preprocessor_neutouch[dataset](time_past, time_future, time_interval)
    ...
    np.savez(f'{dataset}_new.npz', signals=signals, labels=labels, trials=trials)
    print(f'Preprocessing for {dataset} completed with signals.shape = {signals.shape} and labels.shape = {labels.shape}')


#    _   _            _                   _        _____                                                       
#   | \ | |          | |                 | |      |  __ \                                                      
#   |  \| | ___ _   _| |_ ___  _   s_  ___| |__    | |__) | __ ___ _ __  _ __ ___   ___ ___  ___ ___  ___  _ __ 
...
def _preprocess_neutouch_tool(tool_length, time_past, time_future, time_interval):
    ...
    for trial in range(1, 11):
        ...
        df_raw = _read_neutouch_raw(f'/datasets/sensory_ext/data/neutouch/tool_neutouch_1k/trial{trial}_{tool_length}.tact')
        ...
        trials_temp = np.ones_like(labels_temp)*trial
        trials = np.append(trials, trials_temp, axis=0) if trials is not None else trials_temp
    
    return signals, labels, trials


def _preprocess_neutouch_table(time_past, time_future, time_interval):
...
def _preprocess_neutouch_handover(item, time_past, time_future, time_interval):
...
def _preprocess_neutouch_food(item, time_past, time_future, time_interval):
    ...
    for filename in glob.glob(f'/home/shuisong/main_ws/sensory_ext/data/neutouch/food/food_poking_batch[0-9]/{item}_zero_*[0-9].tact'):
        ...
        signal_temp = _bin_neutouch_signal(np.array([start_time]), df_raw, 0, 6, 0.05)
...
def _preprocess_biotac_food(item, samples_past, samples_future):
    ...
    for filename in glob.glob(f'/home/shuisong/main_ws/sensory_ext/data/biotac/food/{item}_zero_*.csv'):
        
        df_raw = pd.read_csv(filename)
        signal_temp = df_raw.pac.values
        trigger_index = np.argmax(np.abs(signal_temp))
        signal_temp = signal_temp[trigger_index-600:trigger_index+600]
