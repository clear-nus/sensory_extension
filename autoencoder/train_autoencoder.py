...
def _generate_tool_autoencoder(signal_type, device):
    ...
    npzfile20 = np.load(f'../preprocessed/{signal_type}_tool_20.npz')
    npzfile30 = np.load(f'../preprocessed/{signal_type}_tool_30.npz')
    npzfile50 = np.load(f'../preprocessed/{signal_type}_tool_50.npz')
    
    scale_factor = 40 if signal_type == 'neutouch' else 1000
    ...
    callbacks = [
        ...
        Checkpoint(dirname=f'models/tool_{signal_type}_autoencoder')
    ]
    ...
    np.save(f'{signal_type}_tool_20.npy', latent20.detach().cpu().numpy())
    np.save(f'{signal_type}_tool_30.npy', latent30.detach().cpu().numpy())
    np.save(f'{signal_type}_tool_50.npy', latent50.detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    _generate_tool_autoencoder('neutouch', 'cuda:0')